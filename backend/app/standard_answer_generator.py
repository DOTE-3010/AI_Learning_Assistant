...
def _technical_description_for(output_format: str, adapter_name: str) -> str:
    ...
    if output_format == "pdf":
        return f"{shared}\nGenerate content that stays friendly to LaTeX conversion."
    return (
...
def _output_contract_for(output_format: str) -> str:
    if output_format == "pdf":
        return (
            "Output contract:\n"
            "- Keep concise section structure.\n"
            "- Avoid markdown-only syntax that harms LaTeX conversion."
        )
    return (
        "Output contract:\n"
        "- Markdown only.\n"
...
def generate_answer_logic(
    ...
    try:
        ...
        error_text = f"Error generating answer: {str(exc)}"
        if return_details:
            return {
